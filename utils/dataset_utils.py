        ...
        for dataset_path in dataset_paths:
            with h5py.File(dataset_path, 'r') as f:
                ...
                for demo in demos:
                    ...
                    # 2. DEBUG PRINTS
                    # We print shapes for index 0 (the reference) and index 34 (the culprit)
                    # or just print all of them to be safe.
                    # print(f"Index {global_index} | File: {dataset_path} | Demo: {demo}")
                    # print(f"  -> Obs shape:    {combined_obs.shape}")

                    # # Optional: Catch the error immediately before the crash
                    # if global_index > 0:
                    #     # Check against the very first element loaded
                    #     first_obs_shape = train_data['obs'][0].shape
                    #     if combined_obs.shape[1] != first_obs_shape[1]:
                    #         print(f"\n[!!!] MISMATCH DETECTED AT INDEX {global_index}")
                    #         print(f"Expected obs dim {first_obs_shape[1]}, got {combined_obs.shape[1]}")
                    #         print(f"Problematic File: {dataset_path}, Demo: {demo}\n")
...
# turtlebot dataset class for loading data from an HDF5 file
class TurtlebotStateDataset(torch.utils.data.Dataset):
    def __init__(self, dataset_paths, pred_horizon, obs_horizon, action_horizon, stats = None):
        # structure initialization
        train_data = {
            'action': [],
        ...
        for dataset_path in dataset_paths:
            with h5py.File(dataset_path, 'r') as f:
                ...
                for demo in demos:
                    ...
                    train_data['action'].append(demo_group['actions'][:])
                    object1 = demo_group['obs/object'][:, :2]
                    object2 = demo_group['obs/object'][:, 2:4]
        ...
        if stats == None:
            stats = {
                'action': get_data_stats(train_data['action']),
                'robot_state': get_data_stats(train_data['obs'][:, :3]),
            }
        ...
        normalized_obs = np.concatenate([
            normalize_data(train_data['obs'][:, :3], stats['robot_state']),
            train_data['obs'][:, 3:],  # objetcs' coordinates are not normalized
        ], axis=-1)
...
class RandomTurtleSceneDataset(torch.utils.data.Dataset):
    def __init__(self, size=1000):
        self.size = size
    ...
    def __getitem__(self, idx):
        # generate a goal
        goal = torch.empty(3).uniform_(0.5, 3.0)
        goal[1] = torch.empty(1).uniform_(-1.5, 1.5)
        goal[2] = 0.0  # fixed z 

        # generate n objects
        objects = []
        for _ in range(4):
            obj = torch.empty(3).uniform_(0.5, 2.5)
            obj[1] = torch.empty(1).uniform_(-1.0, 1.0)
            obj[2] = 0.0
            objects.append(obj)
        objects = torch.stack(objects)  # (4, 3)

        # nobs[:, -1, 3:6] => goal
        # nobs[:, -1, 6:9], [9:12], ... => objects

        nobs = torch.zeros(2, 18)  # obs_horizon=2, dim=25 
        nobs[-1, 3:6] = goal
        for i, obj in enumerate(objects):
            nobs[-1, 6 + i*3: 9 + i*3] = obj
