...
ckpt_path = "output/ckpt/dp_200_2.ckpt"
...
ckpt_path_ld = "output/ckpt/legidiff_200_2.ckpt"
...
# load legibility MLP
film_generator.load_state_dict(torch.load('output/mlp/legibility_2000e_40_2.pt'))
film_generator.eval()
...
with open("stats/stats_200_2.pkl", "rb") as f:
    stats = pickle.load(f)
...
if (success_without== True):
    ...
    if traj_ld is not None:
        ax.plot(traj_ld[:, 0], traj_ld[:, 1], traj_ld[:, 2], color='blue', label='Legibility Diffuser')
