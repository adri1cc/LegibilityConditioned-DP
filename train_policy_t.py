...
for i in range(1, 7): 
    dataset_paths = [
        f"data/t_200_trajectories_{i}.hdf5"
    ]

    print(f"Training on {dataset_paths}")
    ...
    obs_dim = 5 # state (3) + goal coordinates (2)
    action_dim = 3 # next state
    ...
    # Saving checkpoint
    # checkpoint_path = f"output/ckpt/dp_{num_epochs}e_{time.strftime('%Y-%m-%d_%H-%M')}.ckpt" 
    checkpoint_path = f"output/ckpt/t_dp_{i}.ckpt" 
