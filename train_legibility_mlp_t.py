...
from utils.architecture_utils import ControlConditionalUnet1D, FiLMGenerator, ConfigurableFiLMGenerator, SceneEncoder
from utils.dataset_utils import TurtlebotStateDataset
...
for i in range(1, 7):
    ...
    dataset_path = [f"data/t_40_oppos_trajectories_{i}.hdf5"]
    ...
    # encoder.load_state_dict(torch.load('output/scene_encoder/t_se_10000_100e.pth'))
    encoder.eval()
    ...
    # ).to(device)
    film_generator = ConfigurableFiLMGenerator(
        context_dim=64,
        feature_dims={4:1024},
        hidden_dim=1024,
        num_layers=4
    ...
    with tqdm(range(num_epochs), desc='Epoch', disable=True) as tglobal:
        # epoch loop
        for epoch_idx in tglobal:
            epoch_loss = list()
            # batch loop
            with tqdm(dataloader, desc='Batch', leave=False, disable=True) as tepoch:
                for nbatch in tepoch:
                    ...
                    # goal to reach
                    goal_pos = nobs[:, -1, 3:5]
                    goal_pos = F.pad(goal_pos, (0, 1), "constant", 0)

                    # negative goal
                    object_pos = nobs[:, -1, 5:7].unsqueeze(1)
                    object_pos = F.pad(object_pos, (0, 1), "constant", 0)
    ...
    # torch.save(film_generator.state_dict(), f"output/mlp/t_legibility_{num_epochs}e_{time.strftime('%Y-%m-%d_%H-%M')}.pt")
    torch.save(film_generator.state_dict(), f"output/mlp/t_legibility_{num_epochs}e_{i}.pt")
